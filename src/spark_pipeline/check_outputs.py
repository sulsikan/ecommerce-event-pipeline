...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--min-gold-duplicate-summary", type=int, default=1)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        counts = {
            ...
            "gold_duplicate_event_summary": count_parquet(
                spark, args.warehouse_dir / "gold" / "duplicate_event_summary"
            ),
        }
    finally:
        spark.stop()
    ...
    if counts["gold_conversion_funnel_hourly"] < args.min_gold_funnel:
        ...
        return 1
    if counts["gold_duplicate_event_summary"] < args.min_gold_duplicate_summary:
        print(f"min_gold_duplicate_summary={args.min_gold_duplicate_summary}")
        return 1
    return 0
