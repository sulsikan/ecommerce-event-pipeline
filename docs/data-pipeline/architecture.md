...
현재 구현 범위는 Phase 2 Kafka ingestion이다. 로컬 개발 환경에서는 `2019-Oct.csv`를 replay generator로 읽어 Kafka raw topic에 발행하고, 별도 consumer가 PostgreSQL `ecommerce.raw_events`에 저장한다.
...
Kaggle CSV
  ...
  -> Phase 2 Consumer
  -> PostgreSQL ecommerce.raw_events
