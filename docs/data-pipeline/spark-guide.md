...
| Phase 3 | Structured Streaming job과 Gold aggregate 구현 | 실제 분석 목표를 지원하는 streaming 처리 흐름을 만든다. |
| Phase 4 | 품질 실패, quarantine, retry/DLQ 경로를 Spark 결과와 연결 | 실패 데이터가 운영 절차로 이어지게 한다. |
