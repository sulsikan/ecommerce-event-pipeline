...
Phase 4는 Kafka `ecommerce.raw-events` topic을 Spark Structured Streaming으로 읽어 Bronze, Silver, Gold 계층을 로컬 Parquet warehouse에 생성한다. 이 단계의 목표는 모델링이 아니라 Kafka 이후 처리 계층의 책임 경계, checkpoint, event-time 집계, KST 파티셔닝을 검증하는 것이다.
...
python3 scripts/replay-events-to-kafka.py \
  ...
  --speed max \
  --limit 100 \
  --replay-run-id phase4-spark-verify-100
```
...
| Gold | `data/spark-warehouse/gold/duplicate_event_summary` | 5분 단위 중복 이벤트 요약 |
